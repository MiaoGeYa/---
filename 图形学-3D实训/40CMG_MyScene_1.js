...
// 创建纹理对象，加载纹理图  参数为文件路径、纹理图格式(gl.RGB、gl.RGBA等)
function loadTexture(path, format, mipmapping)
{
	...
	image.onload = function()
	{
		...
		if(textureLoaded == numTextures) 
		{
			...
			{
				clearInterval(tick);
				hud.style.backgroundColor="rgb(0,0,0,0)";
			},30)
...
var Obj = function()
{
	...
	this.material = new MaterialObj(); // 对象颜色，默认为白色
...
Obj.prototype.initBuffers = function()
{
	...
	this.vertices.length = 0;  // 顶点数据已传至GPU端，可释放内存
...
{	
	...
	obj.material.diffuse   = vec3(0.4, 0.5, 0.4); 	// 漫反射系数
...
function buildSphere(radius, columns, rows){
	...
	// 设置球材质
	obj.material.ambient = vec3(0.1, 0.1, 0.1); // 环境反射系数
	obj.material.diffuse = vec3(0.5, 0.5, 0.7); // 漫反射系数
	obj.material.specular = vec3(0.5, 0.3, 0.8);// 镜面反射系数
	obj.material.emission = vec3(0.0, 0.0, 0.0);// 发射光
...
function buildTorus(majorRadius, minorRadius, numMajor, numMinor){
	...
	// 设置圆环材质
	obj.material.ambient = vec3(0.1, 0.1, 0.1); // 环境反射系数
	obj.material.diffuse = vec3(0.9, 0.5, 0.5); // 漫反射系数
...
window.onload = function main()
{
	...
	var fogColor=new Float32Array([0.137,0.231,0.323]);
	var fogDist=new Float32Array([3,10]);
	...
	matCamera = mult(translate(0.0,0.0,-2.0),matCamera);
...
function draw2D(ctx)
{
	...
	//用白色的线条绘制三角形
	ctx.beginPath();
	ctx.moveTo(155,20);ctx.lineTo(280,120);
	ctx.lineTo(280,120);ctx.lineTo(20,120);
	ctx.closePath();
	...
	ctx.fillText('姓名：蔡妙格',107,75);
	ctx.fillText('学号：2018031701040',80,105);
	//console.log("绘制名字");	
}
...
function render() 
{
	...
	matMV = mult(matMV, translate(0.5, 0.0, -1.0));
	//matMV=mult(matMV,rotateY(-yRot*3.0));
	matMV=mult(matMV,scale(0.009, 0.009, 0.009));
	gl.uniformMatrix4fv(programObj.u_ModelView, false,
	    flatten(matMV)); //传MV矩阵
	gl.uniformMatrix3fv(programObj.u_NormalMat, false,
	    flatten(normalMatrix(matMV))) ; //传法向矩阵
