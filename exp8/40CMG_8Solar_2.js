...
var hourOfDayX = 0.0;		// X行星一天中的小时数，初始为0
var dayOfYearX  = 0.0;		// X行星一年中的天数，初始为0

var hourOfDayS = 0.0;		// 星星一天中的小时数，初始为0
var dayOfYearS = 0.0;		// 星星一年中的天数，初始为0
...
for(var i=0; i<10; i++)
{
	x.push(20*Math.random());
	y.push(25*Math.random());
}
...
function drawSolarSystem(mvp)
{
	...
	mvp = mult(mvp, scale(1.2,1.2,1.2)); // 控制小太阳大小的缩放变换
	...
	// 用于控制行星X和太阳之间距离的平移变换
    mvp = mult(mvp, translate(6.5, -0.5, 0.0));
	// 绘制X球系统
	drawXSystem(mvp);
	mvp = mvpStack.pop(); // 出栈
	...
	//40度倾斜
	mvp = mult(mvp, rotateY(-360.0 * dayOfYear / 365.0));
	//mvp = mult(mvp, rotateZ(-30));
	//mvp = mult(mvp, rotateZ(45));
	// 绘制地球系统
	drawEarthSystem(mvp);
	mvp = mvpStack.pop(); // 出栈
	...
	mvp = mult(mvp, translate(0.0, -0.05, 0.0));
	mvp = mult(mvp, rotateX(10));
	//mvp = mult(mvp, rotateZ(-15));
	mvp = mult(mvp, rotateY(360.0 * dayOfYearS / 600.0));
	drawStartsSystem(mvp,8.0);
	drawStartsSystem(mvp,5.0);
	drawStartsSystem(mvp,2.0);
...
function drawMoonSystem(mvp)
{
	...
	mvp = mult(mvp,rotateX(90));  //调整南北极为y轴方向
	mvp = mult(mvp, scale(0.05, 0.05, 0.05));// 控制同步卫星大小的缩放变换
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
...
function drawXSystem(mvp)
{
	//绘制行星X
	// 同步卫星旋转速度与行星X 一致
	mvpStack.push(mvp); // 进栈
	mvp = mult(mvp, rotateY(360.0 * hourOfDayX / 36.0));
	mvp = mult(mvp,rotateX(90));  //调整南北极为y轴方向
	mvp = mult(mvp, scale(0.4, 0.4, 0.4));// 控制行星X大小的缩放变换
	// 画一个浅蓝色的球来表示行星X
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	...
	//绘制行星X的逆向同步卫星
	mvp = mult(mvp, rotateY(-8 * 360.0 * dayOfYearX / 300.0));
    mvp = mult(mvp, translate( 1.0, 0.0, 0.0 ));
	mvp = mult(mvp,rotateX(90));  //调整南北极为y轴方向
	mvp = mult(mvp, scale(0.1, 0.1, 0.1));// 控制同步卫星大小的缩放变换
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 1.0, 0.2, 0.2 ); 
	gl.drawArrays(gl.LINES, 0, numVertices);
...
//绘制星星
// 参数为模视投影矩阵
function drawSart(mvp){
	
	mvpStack.push(mvp); // 保存矩阵状态
	mvp = mult(mvp, scale(55, 50, 55));// 控制大小的缩放变换
	mvp=mult(mvp,rotateX(90));
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 1.0, 1.0, 1.0 );  // 灰色
	gl.drawArrays(gl.POINTS, 0, numVertices);
	mvp = mvpStack.pop();
	
	mvpStack.push(mvp); // 保存矩阵状态
	mvp = mult(mvp, scale(40, 40, 55));// 控制大小的缩放变换
	...
	mvpStack.push(mvp); // 保存矩阵状态
	mvp = mult(mvp, scale(30, 30, 35));// 控制大小的缩放变换
	mvp=mult(mvp,rotateZ(-40));
	mvp=mult(mvp,rotateX(90));
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 1.0, 1.0, 1.0 );  // 灰色
...
//绘制星星带
function drawStartsSystem(mvp,n)
{
	...
	for(var i=0.0; i<360.0 ;i++)
	{
		...
		mvp = mult(mvp, translate(n + R[i], 0.0, 0.0));
...
function render()
{
	...
	mvp = mult(mvp,lookAt(vec3(0,2,25),vec3(0,0,0),vec3(0,1,-1)));
	// 在观察坐标系(照相机坐标系)下思考，
