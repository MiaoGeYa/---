...
function render() {
	...
	//matMVP = mult(matMVP,scale(3.0,20.0,3.0));
	drwaCube(scale(3.0,yScale,3.0));

	//蓝色
	gl.uniform4f(u_StartColor,0,0,0,1);
	gl.uniform4f(u_EndColor,0,1,1,1);
	drwaCube(mult(translate(-5, 0, 0),mult(rotateX(thetaLeft),scale(2,10,10))));

	//黄色
	gl.uniform4f(u_StartColor,0,0,0,1);
	gl.uniform4f(u_EndColor,1,1,0,1);
	drwaCube(mult(scale(6,6,6),mult(rotateY(thetaLeft),translate(2, 0, 0))));
