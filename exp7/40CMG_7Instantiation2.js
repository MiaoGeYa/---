...
window.onload = function main(){
	...
	/*设置WebGL相关属性*/
	// 设置视口，占满整个canvas
    gl.clearColor(1.0, 0.87, 0.67, 1.0); // 设置背景色为白色
...
function animation(){
	...
	//bule
	thetaLeft += deltaLeft * elapsed/1000;
...
function render() {
	...
	var a = 13;
	var b = -13;
	gl.uniform4f(u_StartColor,0.9,0.9,0.98,1);
	gl.uniform4f(u_EndColor,0.96,0.96,0.96,1);
	//转动的东西
	drwaCube(mult(translate(-a, a, 0),mult(scale(2,2,2),rotateY(thetaLeft))));
	drwaCube(mult(translate(-a, a, 0),mult(scale(1.0,2.5,1.0),rotateX(thetaLeft))));
	drwaCube(mult(translate(-a, a, 0),scale(0.5,4,0.5)));
	
	drwaCube(mult(translate(-b, b, 0),mult(scale(2,2,2),rotateY(thetaLeft))));
	drwaCube(mult(translate(-b, b, 0),mult(scale(1.0,2.5,1.0),rotateX(thetaLeft))));
	drwaCube(mult(translate(-b, b, 0),scale(0.5,4,0.5)));
	
	//领结
	drwaCube(mult(translate(0, -4.5, 0),scale(5,1.5,4)));
	drwaCube(mult(translate(0, -5, 0),scale(3,2,3.5)));
	drwaCube(mult(translate(0, -7, 0),scale(1.5,5,3.5)));

	//头
	gl.uniform4f(u_StartColor,0,0,0,1);
	gl.uniform4f(u_EndColor,0.8,0.36,0.36,1);
	drwaCube(mult(translate(-7, 2, 0),scale(1.2,12,2)));  //ZUO
	drwaCube(mult(translate(7, 2, 0),scale(1.2,12,2)));//drwaCube(mult(translate(-yScale, -15, 0),scale(5,1,2)));  //SHANG  
	drwaCube(mult(translate(0, 8.2, 0),scale(15.2,1.2,2.2)));
	drwaCube(mult(translate(0, -3.5, 0),scale(15.2,1.2,2.2)));
	
	//帽子
	gl.uniform4f(u_StartColor,0,0,0,1);
	gl.uniform4f(u_EndColor,0,0,0,1);
	drwaCube(mult(translate(0, 9, 0),scale(10,4,2.5)));
	drwaCube(mult(translate(0, 11, 0),mult(rotateY(thetaLeft),scale(12,1,10))));
	
	//眼睛
	drwaCube(mult(translate(-3, 5.5, 0),scale(4,0.5,0.2)));
	drwaCube(mult(translate(-3, 3, 0),scale(3,0.5,0.2)));
	drwaCube(mult(translate(-4.5, 4, 0),scale(0.5,2.5,0.2)));
	drwaCube(mult(translate(-1.5, 4, 0),scale(0.5,2.5,0.2)));
	drwaCube(mult(translate(-3, 3.9, 0),scale(1.5,1.3,0.2)));
	...
	//衣服
	drwaCube(mult(translate(0, -8.5, 0),scale(10,7,3)));
	//鼻子
	drwaCube(mult(translate(0, 2.3, 0),scale(0.5,2.5,0.2)));
	//嘴巴
	drwaCube(mult(translate(0, -1, 0),mult(rotateX(thetaLeft),scale(3,0.6,0.4))));
	//腮红
	gl.uniform4f(u_StartColor,1,0,0,1);
	gl.uniform4f(u_EndColor,1,0,0,1);
	drwaCube(mult(translate(-4.5, 1, 0),mult(rotateX(thetaLeft),scale(3,1,1))));
	drwaCube(mult(translate(4.5, 1, 0),mult(rotateX(thetaLeft),scale(3,1,1))));
	
	requestAnimFrame(render); // 请求重绘
}

function drwaCube(matInstance)
{
	...
	//后面
	gl.uniformMatrix4fv(u_MVPMatrix,false,flatten(mult(matNew,mult(translate(0,0,-0.5),rotateX(180)))));
	gl.drawElements(
		gl.TRIANGLES, 		// 绘制图元类型
		nIndexCount,		// 顶点索引数
		gl.UNSIGNED_SHORT,	// 索引数组元素类型
	...
	//左
	gl.uniformMatrix4fv(u_MVPMatrix,false,flatten(mult(matNew,mult(translate(-0.5,0,0.0),rotateY(-90)))));
	gl.drawElements(
		gl.TRIANGLES, 		// 绘制图元类型
		nIndexCount,		// 顶点索引数
		gl.UNSIGNED_SHORT,	// 索引数组元素类型
	...
	//上
	gl.uniformMatrix4fv(u_MVPMatrix,false,flatten(mult(matNew,mult(translate(0.0,0.5,0.0),rotateX(-90)))));
	gl.drawElements(
		gl.TRIANGLES, 		// 绘制图元类型
		nIndexCount,		// 顶点索引数
		gl.UNSIGNED_SHORT,	// 索引数组元素类型
